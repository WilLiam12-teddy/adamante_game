...
-- Tradução intllib
urbos.intllib = {}
urbos.intllib.S, urbos.intllib.NS = dofile(modpath.."/lib/intllib.lua")

-- Configura tradutor opicional
urbos.S = urbos.intllib.S
urbos.NS = urbos.intllib.NS
...
-- Gera arquivos de tradução urbos.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por urbos apartir de "..file.."\n# textdomain: urbos\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/urbos."..lang_code..".tr", "w")
...
if minetest.get_translator ~= nil then
	s = minetest.get_translator("urbos")
else
	s = urbos.intllib.S
end
...
urbos.s = function(...)
	...
	minetest.log("error", "[urbos] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
	...
	if type(args[1]) == "table" then
		...
		for n,a in ipairs(args[1]) do
			if n ~= 1 then -- Não traduz o primeiro
				table.insert(r, urbos.S(a))
			else
				table.insert(r, a)
			end
		end
		
		return urbos.s(unpack(r))
...
urbos.Sfake = function(s) return s end
