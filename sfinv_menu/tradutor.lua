...
-- Tradução intllib
sfinv_menu.intllib = {}
sfinv_menu.intllib.S, sfinv_menu.intllib.NS = dofile(modpath.."/lib/intllib.lua")

-- Configura tradutor opicional
sfinv_menu.S = sfinv_menu.intllib.S
sfinv_menu.NS = sfinv_menu.intllib.NS
...
-- Gera arquivos de tradução sfinv_menu.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por sfinv_menu apartir de "..file.."\n# textdomain: sfinv_menu\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/sfinv_menu."..lang_code..".tr", "w")
...
if minetest.get_translator ~= nil then
	s = minetest.get_translator("sfinv_menu")
else
	s = sfinv_menu.intllib.S
end

sfinv_menu.s = function(...)
	local args = { ... }
	if pt_to_en[args[1]] ~= nil then
		return s(pt_to_en[args[1]], unpack(args, 2))
	end
	minetest.log("error", "[sfinv_menu] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
	...
	if type(args[1]) == "table" then
		...
		for n,a in ipairs(args[1]) do
			if n ~= 1 then -- Não traduz o primeiro
				table.insert(r, sfinv_menu.S(a))
			else
				table.insert(r, a)
			end
		end
		
		return sfinv_menu.s(unpack(r))
...
sfinv_menu.Sfake = function(s) return s end
